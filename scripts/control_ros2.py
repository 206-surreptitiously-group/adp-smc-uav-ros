...
from fntsmc_ppo_ros import fntsmc_ppo_ros
...
if __name__ == "__main__":
	agent = fntsmc_ppo_ros(pos_ctrl_param)
	...
	while not rospy.is_shutdown():
		...
		if agent.global_flag == 1:  # approaching
			...
			if agent.controller == 'PX4-PID':
				...
				if agent.controller == 'RL':
					pos_s = np.concatenate((e, de))
					param_pos = opt_pos.evaluate(pos_norm(pos_s))  # new position control parameter
					fntsmc_ctrl.get_param_from_actor(param_pos, update_k2=False, update_z=False)  # update position control parameter
